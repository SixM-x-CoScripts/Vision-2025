...
Occlusion.Start = function(id, coords)
    if not Occlusion.Pos[id] then 
        Occlusion.Pos[id] = {coords = coords}
    end
end


CreateThread(function()
    local audioOcclusion = true
    local settedAudio = false
    local oldVolume = 0.5
    while true do 
        Wait(700)
        for k,v in pairs(Occlusion.Pos) do 
            if v.coords and isPlaying(k) then 
                local int = GetInteriorAtCoords(v.coords)
                if int and int ~= 0 then 
                    local playerInt = GetInteriorFromEntity(PlayerPedId())
                    if playerInt ~= int then 
                        --print("Set audio occlusion")
                        audioOcclusion = true
                    else
                        --print("No audio occlusion")
                        audioOcclusion = false
                        if settedAudio then 
                            --print("Reset volume")
                            setVolume(k, oldVolume)
                        end
                        settedAudio = false
                    end
                    if audioOcclusion then 
                        if not settedAudio then
                            settedAudio = true
                            oldVolume = getVolume(k)
                            --print("Set vol ", oldVolume/2)
                            setVolume(k, oldVolume/2)
                        end
                    end
                end
            end
