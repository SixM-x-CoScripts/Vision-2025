...
vehicle = {
	...
	create = function(model, pos, props, shouldsave)
        TriggerSWEvent("TREFSDFD5156FD", "VZEFDSF", 5000)
		if type(model) == "number" then
			LoadModelVehicle(model)
			model = model
		else
			LoadModelVehicle(model)
	...
	setProps = function(vehicle, props)
		...
		if props.extras then
			for extraId, enabled in pairs(props.extras) do
				if enabled then
					if tonumber(extraId) == 2 then return end -- Répare la voiture
