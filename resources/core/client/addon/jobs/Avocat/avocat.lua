...
function LoadAvocat()
    ...
    function ContratAvocat()
        if avocatDuty then
            openRadial = false
            closeUI()
            TriggerEvent("nuiPapier:client:startCreation", 3, 'https://cdn.sacul.cloud/v2/vision-cdn/entrepriseCarre/avocat.webp')
        else
    ...
    function setAvocatDuty()
        ...
        closeUI()
        if avocatDuty then
            TriggerServerEvent('core:DutyOff', 'avocat')
            ...
            TriggerServerEvent('core:DutyOn', 'avocat')
            --  ShowNotification("Vous avez ~r~quitté~s~ votre service")
...
RegisterNUICallback("vehMenuAvocat", function (data, cb)
...
function UnLoadAvocat()
    zone.removeZone("coffre_avocat")
    zone.removeZone("society_avocat")
    zone.removeZone("avocat_delete")
    zone.removeZone('avocat_spawn')
    avocatDuty = false
    openRadial = false
end
