...
CreateThread(function()
    ...
    local pedsData = {
        { model = "s_f_y_scrubs_01",   position = vector3(-250.46, 6323.19, 31.45),                        heading = 221.6,           scenario = "WORLD_HUMAN_CLIPBOARD" },
        { model = "s_m_m_armoured_02", position = vector3(-274.40087890625, 6311.4565429688, 31.48632),    heading = 316.04809570313, scenario = "WORLD_HUMAN_GUARD_STAND" },
        { model = "s_m_m_armoured_02", position = vector3(-254.29682922363, 6315.9135742188, 36.61605072), heading = 315.04809570313, scenario = "WORLD_HUMAN_CLIPBOARD" },
    }
...
function LoadBCMSJob()
    ...
    zone.addZone("pharmacie_bcms",
        vector3(-250.46, 6323.19, 33.45),
    ...
    zone.addZone(
        "spawn_bcms_heli",
        vector3(-254.3, 6315.91, 38.61),
        "Appuyez sur ~INPUT_CONTEXT~ pour accéder au Garage",
        function()
    ...
    zone.addZone(
        "spawn_bcms",
        vector3(-274.40087890625, 6311.4565429688, 33.486328125),
        "Appuyez sur ~INPUT_CONTEXT~ pour accéder au Garage",
        function()
            spawnMenuChoose = 'car'
            OpenMenuVehEms()
    ...
    -- DEV
    zone.addZone("bcms_vest",
        vector3(-259.25, 6323.04, 32.45),
    ...
    function RenfortBCMS()
        if BCMSDuty then
            openRadial = false
            closeUI()
            TriggerSecurEvent('core:makeCall', "bcms", p:pos(), false, "Besoin de renfort")
        else
            exports['vNotif']:createNotification({
                type = 'ROUGE',
...
RegisterNUICallback("Menu_BCMS_vehicule_callback", function(data, cb)
    if vehicle.IsSpawnPointClear(vector3(-271.79919433594, 6313.5556640625, 31.360198974609), 5.0) then
        ...
        if data.name == 'lsfd3' then
            SetVehicleLivery(vehs, 2)
        elseif data.name == 'ambulance4' then
            SetVehicleLivery(vehs, 2)
        elseif data.name == 'emsnspeedo' then
