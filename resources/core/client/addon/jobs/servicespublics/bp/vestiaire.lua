local token = nil

TriggerEvent("core:RequestTokenAcces", "core", function(t)
    token = t
end)

RegisterNUICallback("progressMenuBP", function(data)
    local gradeTenue = nil
    local typeTenue = nil 
    local tenue = nil
        if data.sousCategory == "Tenues" then
        gradeTenue = data.TenueGrade
        typeTenue = data.label
        if p:isMale() then
            tenue = listGradeM_BP[gradeTenue].tenues[typeTenue]
        else
            tenue = listGradeF_BP[gradeTenue].tenues[typeTenue]
        end
        TriggerEvent('skinchanger:change', "shoes_1", tenue.chaussures)
        TriggerEvent('skinchanger:change', "shoes_2", tenue.varChaussures)

        TriggerEvent('skinchanger:change', "bags_1", tenue.sac)
        TriggerEvent('skinchanger:change', "bags_2", tenue.varSac)

        TriggerEvent('skinchanger:change', "decals_1", tenue.decals)
        TriggerEvent('skinchanger:change', "decals_2", tenue.varDecals)

        TriggerEvent('skinchanger:change', "pants_1", tenue.pantalon)
        TriggerEvent('skinchanger:change', "pants_2", tenue.varPantalon)

        TriggerEvent('skinchanger:change', "chain_1", tenue.chaine)
        TriggerEvent('skinchanger:change', "chain_2", tenue.varChaine)
    
        TriggerEvent('skinchanger:change', "torso_1", tenue.haut)
        TriggerEvent('skinchanger:change', "torso_2", tenue.varHaut)

        TriggerEvent('skinchanger:change', "tshirt_1", tenue.sousHaut)
        TriggerEvent('skinchanger:change', "tshirt_2", tenue.varSousHaut)

        TriggerEvent('skinchanger:change', "mask_1", tenue.mask)
        TriggerEvent('skinchanger:change', "mask_2", tenue.varMask)

        TriggerEvent('skinchanger:change', "helmet_1", tenue.helmet)
        TriggerEvent('skinchanger:change', "helmet_2", tenue.varHelmet)

        TriggerEvent('skinchanger:change', "glasses_1", tenue.glasses)
        TriggerEvent('skinchanger:change', "glasses_2", tenue.varGlasses)
    
        if tenue.gpb ~= 0 then
            TriggerEvent('skinchanger:change', "bproof_1", tenue.gpb)
            TriggerEvent('skinchanger:change', "bproof_2", tenue.varGpb)
        else
            TriggerEvent('skinchanger:change', "bproof_1", 0)
            TriggerEvent('skinchanger:change', "bproof_2", 0)
        end

        TriggerEvent('skinchanger:change', "arms", tenue.bras)
        TriggerEvent('skinchanger:change', "arms_2", tenue.varBras)
    end
    if data.name then
        local skin = SkinChangerGetSkin()
        local tenue = {
            ['tshirt_1'] = skin["tshirt_1"],
            ['tshirt_2'] = skin["tshirt_2"],
            ['torso_1'] = skin["torso_1"],
            ['torso_2'] = skin["torso_2"],
            ['decals_1'] = skin["decals_1"],
            ['decals_2'] = skin["decals_2"],
            ['arms'] = skin["arms"],
            ['arms_2'] = skin["arms_2"],
            ['pants_1'] = skin["pants_1"],
            ['pants_2'] = skin["pants_2"],
            ['shoes_1'] = skin["shoes_1"],
            ['shoes_2'] = skin["shoes_2"],
            ['bags_1'] = skin['bags_1'],
            ['bags_2'] = skin['bags_2'],
            ['chain_1'] = skin['chain_1'],
            ['chain_2'] = skin['chain_2'],
            ['helmet_1'] = skin['helmet_1'],
            ['helmet_2'] = skin['helmet_2'],
            ['ears_1'] = skin['ears_1'],
            ['ears_2'] = skin['ears_2'],
            ['mask_1'] = skin['mask_1'],
            ['mask_2'] = skin['mask_2'],
            ['glasses_1'] = skin['glasses_1'],
            ['glasses_2'] = skin['glasses_2'],
            ['bproof_1'] = skin['bproof_1'],
            ['bproof_2'] = skin['bproof_2'],
        }
        exports['vNotif']:createNotification({
            type = 'JAUNE',
            content = "Vous venez de récupérer votre tenue ~s "..data.name
        })
        TriggerSecurGiveEvent("core:addItemToInventory", token, "outfit", 1, {renamed = data.name, data = tenue})
        closeUI()
    end
end)


function LoadBPVestiaire()
    dataVestiaireBP = {}
        dataVestiaireBP = {
            catalogue= {},
            headerIcon= 'https://cdn.sacul.cloud/v2/vision-cdn/icons/market-cart.webp',
            headerIconName= 'Vestiaire',
            callbackName= 'progressMenuBP',
            progressBar= {
                {
                    name= 'Grades'
                }, 
                {
                    name= 'Tenues'
            }
            },
            headerImage= 'https://cdn.sacul.cloud/v2/vision-cdn/headers/header_BP.webp'
        }

        if p:isMale() then
            listGradeM_BP = {
                ["BP Academy"] = {
                    id = 1,
                    tenues = {
                        ["Trainee"] = { 
                            id = 1,
                            haut= 503, varHaut = 19, 
                            sousHaut = 270, varSousHaut = 0, 
                            chaine = 173, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 0, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["Formateur"] = {
                            id = 2,
                            haut= 503, varHaut = 20, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 277, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0,  
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Agent I"] = {
                    id = 2,
                    tenues = {
                        ["AI - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 0, varDecals = 0,
                            helmet = 194, varHelmet = 9,
                            glasses = -1, varGlasses = 0
                        },
                        ["AI - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Agent II"] = {
                    id = 3,
                    tenues = {
                        ["AII - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 0
                        },
                        ["AII - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 0
                        },
                        ["AII - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 146, varDecals = 0
                        },
                        ["AII - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 160, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["AII - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                }, 
                ["Border Agent III"] = {
                    id = 4,
                    tenues = {
                        ["AIII - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 2
                        },
                        ["AIII - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 2
                        },
                        ["AIII - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 159, varDecals = 0
                        },
                        ["AIII - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 160, varDecals = 2,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["AIII - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Supervisor"] = {
                    id = 5,
                    tenues = {
                        ["SUP - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 3
                        },
                        ["SUP - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 160, varDecals = 3
                        },
                        ["SUP - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 147, varDecals = 0
                        },
                        ["SUP - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 160, varDecals = 3,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["SUP - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Officer"] = {
                    id = 6,
                    tenues = {
                        ["BO - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 6
                        },
                        ["BO - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 6
                        },
                        ["BO - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 6
                        },
                        ["BO - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 158, varDecals = 6,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["BO - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["BO - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 6
                        },
                        ["BO - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 6
                        },
                        ["BO - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 6
                        },
                        ["BO - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 6
                        },
                        ["BO - Wintercoat"] = {
                            id = 10,
                            haut= 470, varHaut = 11, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Executive BO"] = {
                    id = 7,
                    tenues = {
                        ["EBO - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 7
                        },
                        ["EBO - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 7
                        },
                        ["EBO - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 7
                        },
                        ["EBO - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 158, varDecals = 7,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["EBO - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["EBO - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 7
                        },
                        ["EBO - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 7
                        },
                        ["EBO - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 7
                        },
                        ["EBO - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 7
                        },
                        ["EBO - Wintercoat"] = {
                            id = 10,
                            haut= 470, varHaut = 11, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 6
                        },
                        ["EBO - PlasticVest"] = {
                            id = 11,
                            haut= 480, varHaut = 17, 
                            sousHaut = 255, varSousHaut = 2, 
                            chaine = 171, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 104, varChaussures = 1, 
                            sac = 114, varSac = 3, 
                            gpb = 72, varGpb = 1, 
                            pantalon = 167, varPantalon = 0,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["EBO - Classe-AS"] = { 
                            id = 12,
                            haut= 488, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 7
                        },
                    }
                },
                ["General BO"] = {
                    id = 8,
                    tenues = {
                        ["GBO - Classe-A"] = { 
                            id = 1,
                            haut= 489, varHaut = 12, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 8
                        },
                        ["GBO - Classe-B"] = {
                            id = 2,
                            haut= 475, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 8
                        },
                        ["GBO - Classe-C"] = {
                            id = 3,
                            haut= 476, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 8
                        },
                        ["GBO - Offroad"] = {
                            id = 4,
                            haut= 495, varHaut = 10, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 71, varGpb = 8, 
                            pantalon = 175, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 158, varDecals = 8,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["GBO - Polo"] = {
                            id = 5,
                            haut= 503, varHaut = 21, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["GBO - Pullover"] = {
                            id = 6,
                            haut= 493, varHaut = 8, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 8
                        },
                        ["GBO - RaincoatG"] = {
                            id = 7,
                            haut= 482, varHaut = 13, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 8
                        },
                        ["GBO - RaincoatY"] = {
                            id = 8,
                            haut= 482, varHaut = 14, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 8
                        },
                        ["GBO - Springcoat"] = {
                            id = 9,
                            haut= 494, varHaut = 10, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 84, varGpb = 0, 
                            pantalon = 175, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 8
                        },
                        ["GBO - Wintercoat"] = {
                            id = 10,
                            haut= 470, varHaut = 11, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["GBO - PlasticVest"] = {
                            id = 11,
                            haut= 480, varHaut = 17, 
                            sousHaut = 255, varSousHaut = 2, 
                            chaine = 171, varChaine = 0, 
                            bras = 4, varBras = 0,
                            chaussures = 104, varChaussures = 1, 
                            sac = 114, varSac = 3, 
                            gpb = 72, varGpb = 1, 
                            pantalon = 167, varPantalon = 0,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["GBO - Classe-AS"] = { 
                            id = 12,
                            haut= 488, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 11, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 184, varSac = 5, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 167, varPantalon = 4,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 150, varDecals = 8
                        },
                    }
                }, 
                ["Specialite"] = {
                    id = 9,
                    tenues = {
                        ["Air"] = { 
                            id = 1,
                            haut= 452, varHaut = 5, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 160, varSac = 0, 
                            gpb = 0, varGpb = 0, 
                            pantalon = 174, varPantalon = 1,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            helmet = 217, varHelmet = 8
                        },
                        ["Marine"] = {
                            id = 2,
                            haut= 503, varHaut = 21, 
                            sousHaut = 280, varSousHaut = 1, 
                            chaine = 172, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 126, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 60, varGpb = 3, 
                            pantalon = 165, varPantalon = 0,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["Revolt"] = {
                            id = 3,
                            haut= 490, varHaut = 3, 
                            sousHaut = 277, varSousHaut = 0, 
                            chaine = 172, varChaine = 0, 
                            bras = 16, varBras = 0,
                            chaussures = 103, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 68, varGpb = 12, 
                            pantalon = 125, varPantalon = 2,
                            mask = 169, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 71, varGlasses = 0,
                            helmet = 205, varHelmet = 11
                        },
                        ["Moto"] = {
                            id = 5,
                            haut= 276, varHaut = 0, 
                            sousHaut = 15, varSousHaut = 0, 
                            chaine = 40, varChaine = 0, 
                            bras = 35, varBras = 0,
                            chaussures = 85, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 17, varGpb = 0, 
                            pantalon = 77, varPantalon = 0,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 0, varGlasses = 0,
                            helmet = 194, varHelmet = 9
                        },
                        ["Cérémonie"] = {
                            id = 6,
                            haut= 178, varHaut = 0, 
                            sousHaut = 15, varSousHaut = 0, 
                            chaine = 33, varChaine = 0, 
                            bras = 75, varBras = 0,
                            chaussures = 85, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 11, varGpb = 0, 
                            pantalon = 77, varPantalon = 0,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 0, varGlasses = 0,
                            helmet = 192, varHelmet = 4
                        },
                        ["Heavy"] = {
                            id = 4,
                            haut= 562, varHaut = 13, 
                            sousHaut = 289, varSousHaut = 0, 
                            chaine = 167, varChaine = 0, 
                            bras = 177, varBras = 0,
                            chaussures = 63, varChaussures = 1, 
                            sac = 0, varSac = 0, 
                            gpb = 75, varGpb = 11, 
                            pantalon = 206, varPantalon = 3,
                            mask = 169, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 41, varGlasses = 0,
                            helmet = 104, varHelmet = 15
                        },
                    }
                },               
            }               
            for k,v in pairs(listGradeM_BP) do
                table.insert(dataVestiaireBP.catalogue, {
                        id= v.id,
                        label = k,
                        image= "https://cdn.sacul.cloud/v2/vision-cdn/Vestiaire/BP/Grades/"..v.id..".webp",
                        category= "Grades",
                        subCategory = "Grades",
                        idVariation= v.id,
                })
                for i,j in pairs(v.tenues) do
                    if j.id ~= nil then
                        table.insert(dataVestiaireBP.catalogue, {
                            id= j.id,
                            label = i,
                            image= "https://cdn.sacul.cloud/v2/vision-cdn/Vestiaire/BP/Tenues/Homme/"..v.id.."/"..j.id..".webp",
                            category= "Tenues",
                            sousCategory= "Tenues",
                            idVariation= j.id,
                            targetId = v.id,
                            TenueGrade = k,
                        })
                    end
                end
            end
        else
            listGradeF_BP = {
                ["BP Academy"] = {
                    id = 1,
                    tenues = {
                        ["Trainee"] = { 
                            id = 1,
                            haut= 517, varHaut = 18, 
                            sousHaut = 291, varSousHaut = 0, 
                            chaine = 142, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 0, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["Formateur"] = {
                            id = 2,
                            haut= 517, varHaut = 19, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Agent I"] = {
                    id = 2,
                    tenues = {
                        ["AI - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["AI - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AI - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Agent II"] = {
                    id = 3,
                    tenues = {
                        ["AII - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 0
                        },
                        ["AII - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 0
                        },
                        ["AII - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1,
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 158, varDecals = 0
                        },
                        ["AII - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 172, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["AII - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AII - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                }, 
                ["Border Agent III"] = {
                    id = 4,
                    tenues = {
                        ["AIII - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 2
                        },
                        ["AIII - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 2
                        },
                        ["AIII - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 171, varDecals = 0
                        },
                        ["AIII - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 172, varDecals = 2,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["AIII - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["AIII - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Supervisor"] = {
                    id = 5,
                    tenues = {
                        ["SUP - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 3
                        },
                        ["SUP - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 3
                        },
                        ["SUP - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 159, varDecals = 0
                        },
                        ["SUP - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 172, varDecals = 3,
                            helmet = 194, varHelmet = 7
                        },
                        ["SUP - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["SUP - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Border Officer"] = {
                    id = 6,
                    tenues = {
                        ["BO - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 6
                        },
                        ["BO - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 6
                        },
                        ["BO - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 6
                        },
                        ["BO - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 6,
                            helmet = 194, varHelmet = 7
                        },
                        ["BO - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["BO - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 6
                        },
                        ["BO - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 6
                        },
                        ["BO - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 6
                        },
                        ["BO - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 6
                        },
                        ["BO - Winter Vest"] = {
                            id = 10,
                            haut= 500, varHaut = 11, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["Executive BO"] = {
                    id = 7,
                    tenues = {
                        ["EBO - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 7
                        },
                        ["EBO - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 7
                        },
                        ["EBO - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 7
                        },
                        ["EBO - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["EBO - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["EBO - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 7
                        },
                        ["EBO - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 7
                        },
                        ["EBO - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 7
                        },
                        ["EBO - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 7
                        },
                        ["EBO - Plastic Vest"] = {
                            id = 10,
                            haut= 506, varHaut = 18, 
                            sousHaut = 217, varSousHaut = 2, 
                            chaine = 140, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 71, varGpb = 1, 
                            pantalon = 178, varPantalon = 1,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["EBO - Winter Vest"] = {
                            id = 11,
                            haut= 500, varHaut = 11, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                },
                ["General BO"] = {
                    id = 8,
                    tenues = {
                        ["GBO - Classe-A"] = { 
                            id = 1,
                            haut= 525, varHaut = 16, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 8
                        },
                        ["GBO - Classe-B"] = {
                            id = 2,
                            haut= 512, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 8
                        },
                        ["GBO - Classe-C"] = {
                            id = 3,
                            haut= 513, varHaut = 3, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 9, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 8
                        },
                        ["GBO - Offroad"] = {
                            id = 4,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 111, varSac = 0, 
                            gpb = 67, varGpb = 8, 
                            pantalon = 181, varPantalon = 8,
                            mask = 220, varMask = 0,
                            decals = 170, varDecals = 8,
                            glasses = -1, varHelemt = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["GBO - Polo"] = {
                            id = 5,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 14, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["GBO - Pullover"] = {
                            id = 6,
                            haut= 510, varHaut = 8, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 62, varGpb = 1, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 170, varDecals = 8
                        },
                        ["GBO - RaincoatG"] = {
                            id = 7,
                            haut= 519, varHaut = 13, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 8
                        },
                        ["GBO - RaincoatY"] = {
                            id = 8,
                            haut= 519, varHaut = 14, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 283, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 8
                        },
                        ["GBO - Springcoat"] = {
                            id = 9,
                            haut= 524, varHaut = 8, 
                            sousHaut = 288, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 78, varGpb = 10, 
                            pantalon = 181, varPantalon = 8,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 162, varDecals = 8
                        },
                        ["GBO - Plastic Vest"] = {
                            id = 10,
                            haut= 506, varHaut = 18, 
                            sousHaut = 217, varSousHaut = 2, 
                            chaine = 140, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 149, varSac = 2, 
                            gpb = 71, varGpb = 1, 
                            pantalon = 178, varPantalon = 1,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                        ["GBO - Winter Vest"] = {
                            id = 11,
                            haut= 500, varHaut = 11, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 3, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 113, varSac = 4, 
                            gpb = 61, varGpb = 0, 
                            pantalon = 187, varPantalon = 2,
                            mask = 0, varMask = 0,
                            helmet = -1, varHelmet = 0,
                            glasses = -1, varHelemt = 0,
                            decals = 0, varDecals = 0
                        },
                    }
                }, 
                ["Specialite"] = {
                    id = 9,
                    tenues = {
                        ["Air"] = { 
                            id = 1,
                            haut= 485, varHaut = 5, 
                            sousHaut = 252, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 17, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 155, varSac = 0, 
                            gpb = 0, varGpb = 0, 
                            pantalon = 180, varPantalon = 1,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = 214, varHelmet = 6
                        },
                        ["Marine"] = {
                            id = 2,
                            haut= 517, varHaut = 20, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 0, varBras = 0,
                            chaussures = 130, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 60, varGpb = 5, 
                            pantalon = 171, varPantalon = 1,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = -1, varHelemt = 0,
                            helmet = -1, varHelmet = 8
                        },
                        ["Revolt"] = {
                            id = 3,
                            haut= 528, varHaut = 10, 
                            sousHaut = 283, varSousHaut = 0, 
                            chaine = 141, varChaine = 0, 
                            bras = 213, varBras = 0,
                            chaussures = 137, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 69, varGpb = 12, 
                            pantalon = 183, varPantalon = 1,
                            mask = 169, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 71, varGlasses = 0,
                            helmet = 203, varHelmet = 11
                        },
                        ["Cérémonie"] = {
                            id = 5,
                            haut= 180, varHaut = 0, 
                            sousHaut = 15, varSousHaut = 0, 
                            chaine = 16, varChaine = 0, 
                            bras = 88, varBras = 0,
                            chaussures = 88, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 9, varGpb = 0, 
                            pantalon = 79, varPantalon = 0,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 0, varGlasses = 0,
                            helmet = 197, varHelmet = 0
                        },
                        ["Moto"] = {
                            id = 3,
                            haut= 289, varHaut = 0, 
                            sousHaut = 15, varSousHaut = 0, 
                            chaine = 24, varChaine = 0, 
                            bras = 36, varBras = 0,
                            chaussures = 88, varChaussures = 0, 
                            sac = 0, varSac = 0, 
                            gpb = 19, varGpb = 0, 
                            pantalon = 79, varPantalon = 0,
                            mask = 0, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 0, varGlasses = 0,
                            helmet = 194, varHelmet = 7
                        },
                        ["Heavy"] = {
                            id = 4,
                            haut= 584, varHaut = 14, 
                            sousHaut = 268, varSousHaut = 0, 
                            chaine = 136, varChaine = 0, 
                            bras = 213, varBras = 0,
                            chaussures = 66, varChaussures = 1, 
                            sac = 0, varSac = 0, 
                            gpb = 74, varGpb = 10, 
                            pantalon = 202, varPantalon = 3,
                            mask = 169, varMask = 0,
                            decals = 0, varDecals = 0,
                            glasses = 71, varGlasses = 0,
                            helmet = 103, varHelmet = 15
                        },
                    }
                },               
            } 
        for k,v in pairs(listGradeF_BP) do
            table.insert(dataVestiaireBP.catalogue, {
                    id= v.id,
                    label = k,
                    image= "https://cdn.sacul.cloud/v2/vision-cdn/Vestiaire/BP/Grades/"..v.id..".webp",
                    category= "Grades",
                    subCategory = "Grades",
                    idVariation= v.id,
            })
            for i,j in pairs(v.tenues) do
                if j.id ~= nil then
                    table.insert(dataVestiaireBP.catalogue, {
                        id= j.id,
                        label = i,
                        image= "https://cdn.sacul.cloud/v2/vision-cdn/Vestiaire/BP/Tenues/Femme/"..v.id.."/"..j.id..".webp",
                        category= "Tenues",
                        sousCategory= "Tenues",
                        idVariation= j.id,
                        targetId = v.id,
                        TenueGrade = k,
                    })
                end
            end
        end
    end

    Wait(500)
    SendNuiMessage(json.encode({
        type = 'openWebview',
        name = 'MenuProgress',
        data = dataVestiaireBP
    }))
end 