...
local e = {}
function LocalLaboPed(Q, labid, iPed) -- ## quand on sort labo, ça delete le ped
    print("LocalLaboPed", Q, labid, iPed)
    CreateThread(function()
        local PosOffset = Q.location - Q.offset;
        local rota = Q.rotation;
        local SceneDict = Q.SceneDict
        local plyAnim = Q.PlayerAnims
        local items = Q.SceneItems
        local sceneanim = Q.SceneAnims
        local Y = SceneDicts
        ...
        for x, y in pairs(items) do
            ...
            e[x] = CreateObject(a3, PosOffset, true)
            TriggerServerEvent("core:labo:instance", ObjToNet(e[x]), labid)
            SetEntityAsMissionEntity(e[x], true, true)
            a2 = a2 + 1;
            print("e[x]", e[x])
        ...
        LaboTable.AddPed(a6)
        for x, a7 in pairs(sceneanim) do
            local a8 = a4[math.ceil(a1 / 3)]
            local a9 = LaboTable.EntityConfig(e[x], a8, Y, a7)
            LaboTable.AddEntity(a9)
            a1 = a1 + 1
        end
        print("goto fin")
        for n = 1, #a4, 1 do
...
local function LocalCreatePed(Q, labid)
    ...
    local iPed = CreatePed(1, GetHashKey(Q.Ped), Q.location, 255, 1)
    ...
    --table.insert(LaboEntities, iPed)
    while not NetworkHasControlOfEntity(iPed) do Wait(1) print("control entity") end
    LocalLaboPed(Q, labid, iPed)
end

function CreateLaboPeds(typeD, labid)
    TriggerServerEvent("core:labo:rmpeds", labid)
    if DrugsPosesTab[typeD] then
        for k, v in pairs(DrugsPosesTab[typeD]) do
            LocalCreatePed(v, labid)
        end
    else
        print("[^1ERROR Labo^7] Table DrugsPosesTab[typeD] "..typeD.." n'existe pas, MERCI DE LE REPORT")
    end
end