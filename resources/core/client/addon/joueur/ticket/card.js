      ...
      if (price_type == "money") {
        ...
        price > 0
          ? (document.getElementById("price").innerHTML =
              ...
              win_message +
              "</span><br><br><span style='font-size:50px;'>" +
              formattedPrice +
              "</span>")
          : (document.getElementById("price").innerHTML =
              "<span style='color:#B2BABB;text-transform:uppercase;'>" +
              lose_message +
              "</span><br><br><span style='font-size:60px;'>" +
              currency +
              " 0</span>");
      } else {
        document.getElementById("price").innerHTML =
          "<span style='color:#2ECC71'>" +
          win_message +
          "</span><br><span style='font-size:20px;color:#7F8C8D'>" +
          amount +
          "x</span><br><span style='font-size:40px;'>" +
          price +
          "</span>";
      }
...
$(document).on("keyup", function (data) {
  if (data.which == 27) {
    $.post(
      "https://" + resourceName + "/nuiCloseCard",
      JSON.stringify({
        key: key,
        price: price,
        amount: amount,
