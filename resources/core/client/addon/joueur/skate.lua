...
Attached = false
local use = false
RegisterNetEvent("core:UseSkate")
AddEventHandler("core:UseSkate", function ()
    if not use then 
        use = true
        RCCar.Start()
    else
...
RCCar.HandleKeys = function(distanceCheck)
	if distanceCheck <= 1.5 then
        ...
        if not Attached then 
            ShowHelpNotification( "Appuyer sur ~INPUT_VEH_FLY_UNDERCARRIAGE~ pour monter")
        else
            ShowHelpNotification( "Appuyer sur ~INPUT_VEH_FLY_UNDERCARRIAGE~ pour descendre")
        end
	...
	if distanceCheck < Config.LoseConnectionDistance then
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if IsControlPressed(0, 63) and IsControlPressed(0, 72) and not overSpeed then
