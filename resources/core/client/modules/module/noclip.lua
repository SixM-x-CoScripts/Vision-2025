...
local function SetNoClipAttributes(ped, status)
    if status then
...
local function GetTarget(distance)
...
local function SetRotation(x, y, z)
...
local function GetDisabledControlNormalBetween(inputGroup, control1, control2)
...
--     NetworkResurrectLocalPlayer(GetEntityCoords(PlayerPedId()), 0.0, 1, 1)
-- end, false)


-- RegisterCommand("weapon", function(source, args, rawCommand)
--     print(args[1], GetHashKey(args[1]))
--     print(IsModelInCdimage(GetHashKey(args[1])))
--     _GiveWeaponToPed(PlayerPedId(), GetHashKey(args[1]), 255, 0, 1)
-- end, false)