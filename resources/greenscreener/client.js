...
on('playerSpawned', () => {
	setTick(async () => {
		console.log("Healed by the greenscreen script");
		ExecuteCommand("heal")
...
async function CustomResetPedComponents() {

	if (config.debug) console.log(`DEBUG: Resetting Ped Component=s`);
...
function stopWeatherResource() {
	if (config.debug) console.log(`DEBUG: Stopping utils Resource (dropped names and time management)`);
	
	if ((GetResourceState('utils') == 'started')) {
		TriggerServerEvent("stopUtilsResource");

		return true;
	}
	
	if (config.debug) console.log(`DEBUG: hiding player dropped text`);
	ExecuteCommand("hideplayerdroppedtext");

	return false;
};

function startWeatherResource() {
	if (config.debug) console.log(`DEBUG: Starting utils Resource (dropped names and time management)`);
	
	if ((GetResourceState('utils') == 'stopped')) {
		TriggerServerEvent("startUtilsResource");
		
		return true;
	}
	
	if (config.debug) console.log(`DEBUG: showing player dropped text`);
	ExecuteCommand("hideplayerdroppedtext");
...
RegisterCommand('screenshotcurrentped', async (source, args) => {

	args[0] = "11";
	args[1] = "17";
	args[2] = "clothing";
	args[3] = "male";
	args[4] = "{\"fov\": 55, \"rotation\": { \"x\": 0, \"y\": 0, \"z\": 15}, \"zPos\": 0.26}";
...
RegisterCommand('screenshotOffset', async (source, args) => {
	const component = parseInt(args[0]);
	...
	if (gender == 'male') {
		...
		print(`Parameter gender only supports "male" or "female". gender=${gender} is not a valid parameter.`);
		return;
	}
		...
		if (IsModelValid(modelHash)) {
			...
			if (!isNaN(offset) && offset > 0) {
				...
				if (type === 'CLOTHING') {
					...
					if (offset > drawableVariationCount) {
						console.log(`Offset (${offset}) is higher than the drawable count (${drawableVariationCount}) for component ${component}`);
						
						return returnControlsToPlayer(playerId, ped, interval, cam, camInfo);
					}

					for (drawable = offset; drawable < drawableVariationCount; drawable++) {
						const textureVariationCount = GetNumberOfPedTextureVariations(ped, component, drawable);
						SendNUIMessage({
							type: config.cameraSettings[type][component].name,
							value: drawable,
					...
					if (offset > propVariationCount) {
						console.log(`Offset (${offset}) is higher than the drawable count (${propVariationCount}) for prop ${component}`);
						
						return returnControlsToPlayer(playerId, ped, interval, cam, camInfo);
					}

					for (prop = offset; prop < propVariationCount; prop++) {
						const textureVariationCount = GetNumberOfPedPropTextureVariations(ped, component, prop);
...
function returnControlsToPlayer(playerId, ped, interval, cam, camInfo) {
	SetPlayerControl(playerId, true);
	FreezeEntityPosition(ped, false);
	clearInterval(interval);
...
setImmediate(() => {
	emit('chat:addSuggestions', [
		...
		{
			name: '/screenshotcurrentped',
			help: 'Take current ped screenshots',
		},
		{
			name: '/screenshotOffset',
			help: 'generate custom cloting screenshots',
			params: [
				{name:"component", help:"The clothing component to take a screenshot of"},
				{name:"offset", help:"The drawable id to start from"},
				{name:"props/clothing", help:"PROPS or CLOTHING"},
				{name:"male/female", help:"The gender to take a screenshot of"},
