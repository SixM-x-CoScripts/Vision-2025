...
function GetPedAnimation()
    return MostRecentDict, MostRecentAnimation
end
exports('GetPedAnimation', GetPedAnimation)
...
if Config.MenuKeybindEnabled then
    ...
    RegisterKeyMapping("emoteui", "Ouvrir le Menu Emotes", "keyboard", Config.MenuKeybind)
else
...
function EmoteMenuStart(args, hard, textureVariation)
    ...
    elseif etype == "gestes" then
        if RP.GestesEmotes[name] ~= nil then
            OnEmotePlay(RP.GestesEmotes[name], name)
        end
    elseif etype == "activites" then
        if RP.ActivitesEmotes[name] ~= nil then
            OnEmotePlay(RP.ActivitesEmotes[name], name)
        end
    elseif etype == "gang" then
        if RP.GangEmotes[name] ~= nil then
            OnEmotePlay(RP.GangEmotes[name], name)
        end
    elseif etype == "positions" then
        if RP.PositionsEmotes[name] ~= nil then
            OnEmotePlay(RP.PositionsEmotes[name], name)
        end
    elseif etype == "sante" then
        if RP.SanteEmotes[name] ~= nil then
            OnEmotePlay(RP.SanteEmotes[name], name)
        end
    elseif etype == "sport" then
        if RP.SportEmotes[name] ~= nil then
            OnEmotePlay(RP.SportEmotes[name], name)
        end
    elseif etype == "autres" then
        if RP.AutresEmotes[name] ~= nil then
            OnEmotePlay(RP.AutresEmotes[name], name)
        end
    end
end
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3, textureVariation)
    ...
    if GetResourceState("ZeroTrust") == "started" then
        exports["ZeroTrust"]:InsertToWLObj(prop1)
    end
