...
function createLightbarCar(modelName)
    ...
    if currentVehicle == 0 then return print("[vlightbar] ~~ VEHICULE NULL ~~") end
    local ped = PlayerPedId()
    local model, coord, heading = modelName, GetEntityCoords(currentVehicle), -GetEntityHeading(ped)
    loadModel(model)
    TriggerServerEvent("vlightbar:server:spawnLightbarVeh", model, vector3(0.0,0.0,0.0), heading,false,nil,GetVehicleNumberPlateText(currentVehicle), false)
end

function spawnLightbarVeh(veh, model, load,data,pt)
    if data == nil then data = {} end
    local coord = data.coords or vector3(0,0,0)
    local rot = data.rotation  or vector3(0,0,0)
    ...
    if not load then
        ...
        exports["ZeroTrust"]:InsertToWLObj(model)
        AttachEntityToEntity(lightbar, currentVehicle, 0, coord,rot, true, true, false, false, 3, true)
        local vehicleIdentity = GetEntityModel(currentVehicle)
        local newLightbarData = { model = model,
        coordData = { coord = vector3(XCoord, YCoord, ZCoord), rotation = vector3(Xrot, Yrot, Zrot) },
        ...
        TriggerServerEvent("vlighbar:server:attackLightbar", pt.plate, LightbarVehicles[pt.plate], true)
    end
end

function saveNewLightbars()
    newLightbars = {}
    TriggerServerEvent("vlighbar:server:attackLightbar", MainVehiclePlate, CurrentVehicleLightBars)
end
...
function loadLightbarInCar(vehicle, model)
    local plate = GetEntityModel(vehicle)
    local lightbars = LightbarVehicles[plate]
    if lightbars == nil then 
        exports['vNotif']:createNotification({
            type = 'ROUGE',
            content = "Votre véhicule ne peut pas avoir de lightbar."
        })
        return
    end
    for key, value in ipairs(lightbars) do
        ...
        loadModel(model)
        TriggerServerEvent("vlightbar:server:spawnLightbarVeh", model, GetEntityCoords(vehicle), GetEntityHeading(vehicle),true,coords,pc, GetVehicleNumberPlateText(vehicle), true)
    end
end

function searchNearVehicle()
    if not isPermission("user") and not vLightbar.NoNeedPerm then return end
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            local vCoords, plate = GetEntityCoords(nearveh), GetEntityModel(nearveh)
            ...
            if nearveh ~= 0 and isVehicle and not blackListVeh then
                sleep = 5
                ShowFloatingHelpNotificationsc("~INPUT_PICKUP~ Lightbars Menu", vCoords, plate)
...
function controlBlackList(vehName)
    for index, name in ipairs(vLightbar.BlacklistVehicle) do
    ...
    if dt ~= "move-cam" then
        ...
        for index, value in pairs(CurrentVehicleLightBars) do
            if NetToVeh(value.lbEntity) == selectLb then
                value.coordData.rotation = vector3(Xrot, Yrot, Zrot)
                value.coordData.coord = vector3(XCoord, YCoord, ZCoord)
            end
        end
    else
        moveCam(sepeed, typ)
...
function moveCam(sepeed, typ)
    ...
    elseif typ == "pleft" then
        pcamx = pcamx + sepeed
        PointCamAtCoord(cam, GetOffsetFromEntityInWorldCoords(currentVehicle, vector3(pcamx, pcamy, pcamz)))
...
function openLightbarMenu(nearveh)
    displayNui(true)
    MainVehiclePlate = GetEntityModel(nearveh)
end
...
function toggleLightbars(veh)
    ...
    local plate = GetEntityModel(sirenCar)
    ...
    if isDriver and isLightbarCar or veh ~= nil then
        TriggerServerEvent("vlightbar:server:toggleLightbars", VehToNet(sirenCar), plate, GetVehicleNumberPlateText(sirenCar))
    end
end
...
function toggleLightbarAudio()
    ...
    if isDriver and isLightbarCar then
        ...
        TriggerServerEvent("vlightbar:server:toggleLightbarAudio", VehToNet(sirenCar), plate, sirenTon, GetVehicleNumberPlateText(sirenCar))
    end
end
...
function toggleSirenHorn(bool)
    if not vLightbar.useSirenHorn then return end
    local player = PlayerPedId()
    local sirenCar = GetVehiclePedIsIn(player, false)
    local isDriver = (GetPedInVehicleSeat(sirenCar, -1) == player) and true or false
    if not isDriver then return end
    ...
    if isDriver and isLightbarCar then
        if bool then
            ...
            TriggerServerEvent("vlightbar:server:TogglesirenHorn", VehToNet(sirenCar), bool, GetVehicleNumberPlateText(sirenCar))
        else
            TriggerServerEvent("vlightbar:server:TogglesirenHorn", VehToNet(sirenCar), bool, GetVehicleNumberPlateText(sirenCar))
...
function startCam()
    ...
    PointCamAtCoord(cam, GetOffsetFromEntityInWorldCoords(entity, vector3(pcamx, pcamy, pcamz)))
...
function createprop(attachModelSent, boneNumberSent, x, y, z, xR, yR, zR)
    removeAttachedPropPerm()
    exports["ZeroTrust"]:InsertToWLObj(attachModelSent)
...
function loadModel(model)
    if not HasModelLoaded(model) then
        ...
        while not HasModelLoaded(model) do
            print("~~ MODÈLE EN ATTENTE DE CHARGEMENT ~~")
            Citizen.Wait(1)
        end
    end
end
...
function ShowFloatingHelpNotificationsc(msg, coords, name)
    ...
    SetFloatingHelpTextWorldPosition(1, coords + vector3(0, 0, 1.0))
    SetFloatingHelpTextStyle(1, 1, 2, -1, 3, 0)
    BeginTextCommandDisplayHelp('FloatingHelpNotificationsc' .. name)
    EndTextCommandDisplayHelp(2, 0, 0, -1)
end

exports('loadLightbarInCar', loadLightbarInCar)